module outflow_flux
    ...
    real(db), dimension(:), allocatable :: outflow_fluxes, outflow_values, pe_outflow_fluxes, pe_outflow_values, ke_outflow_fluxes,&
        ke_outflow_values, one_integral
    ...
    subroutine finalise_outflow_fluxes()
        ...
        deallocate(outflow_fluxes, outflow_values, pe_outflow_fluxes, pe_outflow_values, ke_outflow_fluxes, ke_outflow_values, &
            one_integral)
    end subroutine
    ...
    subroutine calculate_outflow_fluxes(mesh_data, flow_solution)
        ...
        real(db) :: integral_flux, integral_value, pe_integral_flux, pe_integral_value, ke_integral_flux, ke_integral_value, &
            one_integral_temp
        ...
        do k = 1,no_faces
            ...
            if (bdry_face_no /= 0) then
                integral_flux     = 0.0_db
                integral_value    = 0.0_db
                pe_integral_value = 0.0_db
                pe_integral_flux  = 0.0_db
                ke_integral_value = 0.0_db
                ke_integral_flux  = 0.0_db
                ...
                do qk = 1,no_quad_points
                    ...
                    pe_integral_flux = pe_integral_flux + &
                        quad_weights(qk)*face_jacobian(qk)*dot_product(uh_flow(1:problem_dim), face_normals(:,qk))* &
                            uh_flow(problem_dim+1)

                    pe_integral_value = pe_integral_value + &
                        quad_weights(qk)*face_jacobian(qk)*uh_flow(problem_dim+1)

                    ke_integral_flux = ke_integral_flux + &
                        quad_weights(qk)*face_jacobian(qk)*dot_product(uh_flow(1:problem_dim), face_normals(:,qk))* &
                            0.5_db*abs(dot_product(uh_flow(1:problem_dim), uh_flow(1:problem_dim)))

                    ke_integral_value = ke_integral_value + &
                        quad_weights(qk)*face_jacobian(qk)* &
                            0.5_db*abs(dot_product(uh_flow(1:problem_dim), uh_flow(1:problem_dim)))
                ...
                outflow_fluxes   (bdry_face_no) = outflow_fluxes   (bdry_face_no) + integral_flux
                pe_outflow_fluxes(bdry_face_no) = pe_outflow_fluxes(bdry_face_no) + pe_integral_flux
                ke_outflow_fluxes(bdry_face_no) = ke_outflow_fluxes(bdry_face_no) + ke_integral_flux

                outflow_values   (bdry_face_no) = outflow_values   (bdry_face_no) + integral_value
                pe_outflow_values(bdry_face_no) = pe_outflow_values(bdry_face_no) + pe_integral_value
                ke_outflow_values(bdry_face_no) = ke_outflow_values(bdry_face_no) + ke_integral_value
