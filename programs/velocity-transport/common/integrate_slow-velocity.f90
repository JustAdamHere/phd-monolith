module integrate_slow_velocity
  use aptofem_kernel
  use param
  ...
  function calculate_integral_slow_velocity(mesh_data, velocity_solution, IVS_only, U, V)
    use fe_mesh
    use fe_solution
    ...
    ! BUILD INTEGRAL !
    !!!!!!!!!!!!!!!!!!
    calculate_integral_slow_velocity = 0.0_db
    ...
    do k = 1, no_eles
      ...
      if ((300 <= element_region_id .and. element_region_id <= 399) .or. &
        ...
        do qk = 1, no_quad_points
          ...
          velocity_magnitude = U*sqrt(velocity_magnitude)

          if (velocity_magnitude <= V) then
            calculate_integral_slow_velocity = calculate_integral_slow_velocity + quad_weights_ele(qk) * jacobian(qk) * 1.0_db
          end if
        end do
      end if
    end do
