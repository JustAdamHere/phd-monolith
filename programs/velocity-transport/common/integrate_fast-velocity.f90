module integrate_fast_velocity
  use aptofem_kernel
  use param
  ...
  function calculate_integral_fast_velocity(mesh_data, velocity_solution, IVS_only, U, V)
    use fe_mesh
    use fe_solution
    ...
    real(db)                   :: calculate_integral_fast_velocity
    ...
    ! BUILD INTEGRAL !
    !!!!!!!!!!!!!!!!!!
    calculate_integral_fast_velocity = 0.0_db
      ...
      if ((300 <= element_region_id .and. element_region_id <= 399) .or. &
        ...
        do qk = 1, no_quad_points
          ...
          if (velocity_magnitude >= V) then
            calculate_integral_fast_velocity = calculate_integral_fast_velocity + quad_weights_ele(qk) * jacobian(qk) * 1.0_db
          end if
        end do
      end if
    end do
