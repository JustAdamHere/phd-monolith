module integrate_one
  ...
  function calculate_integral_one(mesh_data, velocity_solution, IVS_only)
    use fe_mesh
    use fe_solution
    ...
    ! Input arguments.
    real(db) :: calculate_integral_one
    ...
    integer                                :: no_eles, no_nodes, no_faces, problem_dim, npinc, no_quad_points_volume_max, &
      no_quad_points_face_max, k, dim_soln_coeff, no_pdes, no_quad_points, element_region_id, qk, min_region
    ...
    do k = 1, no_eles
      ...
      if ((300 <= element_region_id .and. element_region_id <= 399) .or. &
        ...
        do qk = 1, no_quad_points
          calculate_integral_one = calculate_integral_one + quad_weights_ele(qk) * jacobian(qk) * 1.0_db
        end do
      end if
    end do
