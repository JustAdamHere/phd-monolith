module outflow_transport_flux
    ...
    real(db), dimension(:), allocatable :: outflow_transport_fluxes, outflow_transport_values
    ...
    subroutine setup_outflow_transport_fluxes(largest_bdry_face_no)
        ...
        allocate(outflow_transport_values(largest_bdry_face_no))
        outflow_transport_fluxes = 0.0_db
        outflow_transport_values = 0.0_db
    ...
    subroutine finalise_outflow_transport_fluxes()
        ...
        deallocate(outflow_transport_fluxes, outflow_transport_values)
    end subroutine
    ...
    subroutine calculate_outflow_transport_fluxes(mesh_data,flow_solution,transport_solution)
        ...
        real(db) :: integral_flux, integral_value
        ...
        do k = 1,no_faces
            ...
            if (bdry_face_no /= 0) then
                ...
                do qk = 1,no_quad_points
                    ...
                    integral_flux = integral_flux + uh_transport(1)* &
                        dot_product(uh_flow(1:problem_dim), face_normals(:,qk))*quad_weights(qk)*face_jacobian(qk)

                    integral_value = integral_value + uh_transport(1)* &
                        quad_weights(qk)*face_jacobian(qk)
                end do

                outflow_transport_fluxes(bdry_face_no) = outflow_transport_fluxes(bdry_face_no) + integral_flux
                outflow_transport_values(bdry_face_no) = outflow_transport_values(bdry_face_no) + integral_value
            end if
        end do
