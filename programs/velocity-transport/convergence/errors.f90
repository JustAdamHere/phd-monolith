...
subroutine error_norms_transport(errors,mesh_data,soln_data,t)
  ...
  ! Determine analytical solution
          ...
          call anal_soln_velocity(u,global_points_ele(:,qk),problem_dim,no_pdes,0,0.0_db)
          call anal_soln_velocity_1(gradient_u,global_points_ele(:,qk),problem_dim,no_pdes)
    ...
    do k = 1,no_faces
       ...
       if (bdry_face > 0) then
          ...
          do qk = 1,no_quad_points
             call anal_soln_velocity(uh2(:,qk),global_points_face(:,qk),problem_dim,no_pdes,0,0.0_db)
          end do
