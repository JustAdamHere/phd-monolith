program adam_nsku_transport_convergence
  use aptofem_kernel
  ...
  type(solution)                :: solution_transport
  type(fe_assembly_subroutines) :: fe_solver_routines_nsku, fe_solver_routines_transport

  type(refinement_tree)              :: mesh_tree
  integer, dimension(:), allocatable :: refinement_marks
  integer                            :: mesh_no, no_eles

  class(sp_matrix_rhs), pointer   :: sp_matrix_rhs_data_nsku, sp_matrix_rhs_data_transport
  type(default_user_data)         :: scheme_data_nsku, scheme_data_transport
  type(dirk_time_stepping_scheme) :: dirk_scheme_nsku

  character(15), dimension(5) :: errors_format_nsku, errors_name_nsku
  character(15), dimension(4) :: errors_format_transport, errors_name_transport
  real(db), dimension(5)      :: errors_nsku
  real(db), dimension(4)      :: errors_transport
  ...
  !!!!!!!!!!!!!!!!!!
  no_errors_nsku          = 5
  errors_format_nsku(1:5) = '(g15.5)'
  errors_name_nsku(1)     = '||u-u_h||_0'
  errors_name_nsku(2)     = '||p-p_h||_0'
  errors_name_nsku(3)     = '||u-u_h,p-p_h||_0'
  errors_name_nsku(4)     = '||u-u_h,p-p_h||_DG'
  errors_name_nsku(5)     = '||div(u-u_h)||_0'

  no_errors_convection          = 4
  errors_format_convection(1:4) = '(g15.5)'
  errors_name_convection(1)     = '||u-u_h||_{L^2}'
  errors_name_convection(2)     = '|u-u_h|_{H^1}'
  errors_name_convection(3)     = '||u-u_h||_{DG}'
  errors_name_convection(4)     = '|u-u_h|_{H^2}'

  !!!!!!!!!!!!!!!!!!!!
  !! VELOCITY SETUP !!
  !!!!!!!!!!!!!!!!!!!!
  call create_fe_solution(solution_nsku, mesh_data, 'fe_solution_nsku', aptofem_stored_keys, dirichlet_bc_nsku)
  ...
  scheme_data_transport%time_step    = 0.1_db

  call set_current_time(solution_transport, 0.0_db)

  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! 1a. VELOCITY STEADY-STATE SPATIAL CONVERGENCE !!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  call store_subroutine_names(fe_solver_routines_nsku, 'assemble_jac_matrix_element',       jacobian_nsku_ss, 1)
  call store_subroutine_names(fe_solver_routines_nsku, 'assemble_jac_matrix_int_bdry_face', jacobian_face_nsku_ss, 1)
  call store_subroutine_names(fe_solver_routines_nsku, 'assemble_residual_element',         element_residual_nsku_ss, 1)
  call store_subroutine_names(fe_solver_routines_nsku, 'assemble_residual_int_bdry_face',   element_residual_face_nsku_ss, 1)

  call create_mesh(mesh_data,  get_boundary_no_darcy, compute_point_on_boundary, 'Mesh_gen', aptofem_stored_keys)
  ...
  do mesh_no = 1, no_meshes
    ...
    if (ifail) then
      call write_message(io_err, 'Error: Newton solver failed to converge on mesh number' // mesh_no)
      error stop
    end if

    call error_norms_velocity(errors_nsku, mesh_data, solution_nsku)

    if (mesh_no < no_meshes) then
      call hp_mesh_adapt_uniform_refinement('Mesh_Refinement', aptofem_stored_keys, mesh_tree, mesh_data, &
        solution_nsku, mesh_data_orig, solution_nsku_orig, mesh_no)
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! 1b. TRANSPORT STEADY-STATE SPATIAL CONVERGENCE !!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !   call error_norms_transport()
  ! end do

  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! 1c. TRANSPORT STEADY-STATE SPATIAL CONVERGENCE, USING VELOCITY SOLUTION !!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! 2a. VELOCITY TIME-DEPENDENT SPATIAL CONVERGENCE !!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! 2b. TRANSPORT TIME-DEPENDENT SPATIAL CONVERGENCE !!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! 2c. TRANSPORT TIME-DEPENDENT SPATIAL CONVERGENCE, USING VELOCITY SOLUTION !!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! 3a. VELOCITY TIME-DEPENDENT TEMPORAL CONVERGENCE !!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! 3b. TRANSPORT TIME-DEPENDENT TEMPORAL CONVERGENCE !!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! 3c. TRANSPORT TIME-DEPENDENT TEMPORAL CONVERGENCE, USING VELOCITY SOLUTION !!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
